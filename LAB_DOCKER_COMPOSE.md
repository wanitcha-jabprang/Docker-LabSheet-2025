...
**บันทึกผลการทดลอง**
![รูปผลการทดลอง คำสั่ง docker ps](images/docker-ps.jpg)
...
```
#### docker run
ใช้สำหรับสร้างและรัน container ใหม่จาก image ที่กำหนด  
หากในเครื่องยังไม่มี image นั้น Docker จะทำการดาวน์โหลด (pull) ให้อัตโนมัติ

---

#### -d (Detached Mode) 
ใช้รัน container แบบเบื้องหลัง  
- ไม่แสดง log บนหน้าจอ  
- สามารถใช้ terminal ต่อได้ทันที  

---

#### --name my-nginx
ใช้ตั้งชื่อ container เป็น my-nginx  
ช่วยให้เรียกใช้งานได้ง่าย

---

#### -p 8080:80
ใช้สำหรับเชื่อมต่อ port ระหว่างเครื่อง (host) และ container  

รูปแบบ:  
-p <portเครื่อง>:<portใน container>  

ความหมาย:  
- 8080 คือ port ของเครื่อง (host)  
- 80 คือ port ภายใน container (Nginx)  

เมื่อเข้า http://localhost:8080 จะถูกส่งไปยัง Nginx  

---

#### nginx:alpine
เป็น image ที่ใช้ในการสร้าง container  

- nginx คือ Web Server  
- alpine คือเวอร์ชันที่มีขนาดเล็กและเบา  
...
![รูปผลการรันคำสั่ง docker exec -it](<images/docker exec -it.jpg>)
</details>
...
> **📝 บันทึกผล**: Container ID คือ: `37c4c1dd1c973b26d3b9baf0793075da31a935c3b701977d01189241d5100e1c` และ IP Address คือ: `172.17.0.2`
...
![docker run node](images/docker-run-node.png)
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ container ที่กำลังทำงานอยู่ (running)และ docker ps -a แสดง container ทั้งหมด (รวมที่ stop แล้วด้วย)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 5–8 MB (เล็กมาก เพราะใช้ Alpine Linux)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้สำหรับ ลบ container อัตโนมัติหลังหยุดการทำงาน เหมาะกับงานแบบชั่วคราว เช่น run test / script แล้วไม่ต้องเก็บ container ไว้

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it สร้าง container ใหม่แล้วเข้าไปใช้งานทันที ส่วน docker exec -it เข้าไปใช้งานใน container ที่กำลังรันอยู่แล้ว
...
```bash
# 1. ดู container ที่ใช้ image นี้
docker ps -a
# 2. หยุด container (ถ้ามี)
...
```
Multi-stage Build คือการสร้าง Docker image โดยแบ่งขั้นตอนการ build ออกเป็นหลาย stage ภายใน Dockerfile เดียว เพื่อให้สามารถใช้ไฟล์หรือผลลัพธ์จาก stage ก่อนหน้า (เช่น ไฟล์ที่ compile แล้ว) ไปใช้ใน stage ถัดไปได้

ข้อดีคือช่วยลดขนาดของ image เพราะใน stage สุดท้ายจะเลือกนำเฉพาะไฟล์ที่จำเป็นมาใช้งาน ทำให้ไม่ต้องรวมเครื่องมือ build หรือ dependency ที่ไม่จำเป็นเข้าไปใน image สุดท้าย ส่งผลให้ image มีขนาดเล็ก ปลอดภัยมากขึ้น และเหมาะกับการ deploy มากขึ้น
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `699` MB 
...
```
-e DB_PATH=/app/data/booking.db เป็นการกำหนด Environment Variable ให้กับ container โดยตั้งค่า DB_PATH เพื่อบอกตำแหน่งไฟล์ฐานข้อมูลภายใน container ว่าอยู่ที่ /app/data/booking.db

-v $(pwd)/test-data:/app/data เป็นการทำ Volume Mapping (Bind Mount) โดยนำโฟลเดอร์ test-data จากเครื่อง host (ตำแหน่งปัจจุบัน $(pwd)) ไปเชื่อมกับโฟลเดอร์ /app/data ภายใน container
```
...
**บันทึกผลการทดลอง**
![บันทึกรูปผลการรัน curl http://localhost:5000](images/api-doc.png)
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: booking-backend:1.0 คือ ชื่อของ Docker image พร้อม tag เวอร์ชัน โดย booking-backend เป็นชื่อ image และ 1.0 เป็นเวอร์ชันที่ใช้สำหรับระบุหรือแยก image แต่ละเวอร์ชัน

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำให้ backend รองรับ route / ก่อน โดยเพิ่ม endpoint ในโค้ดและต้องรัน container โดย map port ให้ถูกต้อง เช่น -p 5000:5000 จากนั้นจึงสามารถใช้คำสั่ง curl http://localhost:5000 หรือ curl http://localhost (ถ้าใช้ port 80) เพื่อทดสอบ API ได้
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `92.9` MB | booking-backend:1.0 ขนาด `699` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 มีขนาดเล็กกว่า booking-backend:1.0 เพราะ frontend ที่ build เสร็จแล้วมักเหลือเพียงไฟล์ static เช่น HTML, CSS, JavaScript และรันผ่าน nginx ซึ่งมีขนาดไม่ใหญ่ ส่วน backend ต้องมี runtime และ dependency ของฝั่ง server เช่น Node.js, library ต่าง ๆ และโค้ดสำหรับประมวลผล API จึงมักมีขนาดใหญ่กว่า

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: location /api/ ใช้กำหนดว่า ถ้ามี request ที่ขึ้นต้นด้วย /api/ ให้ nginx จัดการตามกฎที่กำหนดไว้ใน block นี้ โดยส่วนใหญ่จะใช้สำหรับ proxy request ไปยัง backend server แทนที่จะให้ nginx หาไฟล์ static ใน frontend

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะเมื่อ container อยู่ใน Docker network เดียวกัน Docker จะมีระบบ DNS ภายในที่ทำให้ container สามารถเรียกหากันด้วย ชื่อ container หรือชื่อ service ได้ ดังนั้น frontend จึงสามารถ proxy ไปที่ backend ได้โดยตรง เช่น http://backend:5000 โดยไม่จำเป็นต้องใช้ localhost หรือ IP address จริง
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือไฟล์สำหรับกำหนดและจัดการหลาย ๆ container พร้อมกันในรูปแบบ configuration เดียว เช่น กำหนด service (frontend, backend, database), network และ volume ทำให้สามารถสั่ง docker compose up เพื่อรันทั้งระบบได้ในคำสั่งเดียว ช่วยให้สะดวก ลดความซับซ้อน และเหมาะกับการพัฒนาและ deploy ระบบที่มีหลาย service

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: หมายถึง container จะถูก restart อัตโนมัติเมื่อเกิดการหยุดทำงาน (เช่น crash หรือ Docker restart) แต่จะ ไม่ restart ถ้าผู้ใช้สั่ง docker stop เอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume เช่น sqlite_data เป็น storage ที่ Docker จัดการให้โดยอัตโนมัติ ไม่ต้องระบุ path จริงบนเครื่อง host ทำให้ใช้งานง่าย ปลอดภัย และพกพาได้ดี ในขณะที่ Bind Mount ต้องระบุ path บนเครื่อง เช่น ./data:/app/data ซึ่งขึ้นกับโครงสร้างไฟล์ของเครื่องนั้น สำหรับ SQLite จำเป็นต้องใช้ volume เพราะ SQLite เก็บข้อมูลเป็นไฟล์ .db หากไม่ใช้ volume ข้อมูลจะหายเมื่อ container ถูกลบ ดังนั้นจึงใช้ Named Volume เพื่อให้ข้อมูลยังคงอยู่และไม่ผูกกับ path ของเครื่องใดเครื่องหนึ่ง
---
...
**บันทึกรูปผลการทดลอง**
![รูปการทดลองสร้าง Booking ใหม่](images/test-build.png)
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: docker compose ps ใช้แสดงสถานะของแต่ละ service ในระบบ เช่น ชื่อ container, image, คำสั่งที่รัน, สถานะ (Up/Exited/healthy) และพอร์ตที่เปิดใช้งาน (port mapping) เพื่อดูว่า service ทำงานอยู่หรือไม่และเข้าถึงผ่านพอร์ตใด

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP Address ของแต่ละ container คือหมายเลข IP ภายใน Docker network ที่ใช้สื่อสารกันระหว่าง container เดียวกัน เช่น frontend สามารถเรียก backend ผ่านชื่อ service (backend) ซึ่ง Docker จะ map ไปยัง IP ภายในนี้โดยอัตโนมัติ

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ไฟล์ SQLite booking.db ถูกสร้างใน path /app/data/booking.db ภายใน container
